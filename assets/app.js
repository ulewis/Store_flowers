(() => {
  ...
  async function apiGet(action, params={}){
    ...
    const url=new URL(cfg.API_URL); url.searchParams.set('action',action); Object.entries(params).forEach(([k,v])=>url.searchParams.set(k,v));
    const res=await fetch(url.toString(),{method:'GET',redirect:'follow'}); if(!res.ok) throw new Error('No se pudo consultar la tienda'); return res.json();
  }
  async function apiPost(action,payload){
    ...
    if(!res.ok) throw new Error('No se pudo registrar la separación'); return res.json();
  }
  async function bootstrap(){
    if(cfg.API_URL){
      try{ const data=await apiGet('bootstrap'); if(data && data.ok!==false){ state.data=normalize(data.data||data); state.apiReady=true; } }
      catch(err){ console.warn('Backend no disponible; se usa catálogo de demostración.',err); }
    }
  ...
  function renderCategories(){
    ...
    $$('.occasion-card').forEach(card=>card.addEventListener('click',()=>{ state.category=state.category===card.dataset.category?'':card.dataset.category; renderCategories(); renderProducts(); $('#productos').scrollIntoView({behavior:'smooth'}); }));
  }
  ...
  function renderProducts(){
    ...
    const active=state.category?categoryById(state.category):null; $('#activeFilter').hidden=!active; if(active) $('#activeFilter').textContent=`${active.emoji||''} ${active.nombre} · tocar categoría nuevamente para limpiar`;
  ...
  function cartQty(id,personalization=''){ return state.cart.find(i=>i.id===id&&i.personalization===personalization)?.qty||0; }
  function addToCart(id,qty=1,personalization=''){
    const p=state.data.products.find(x=>x.id===id); if(!p||p.available_stock<=0) return toast('Este producto está agotado.');
    const existing=state.cart.find(i=>i.id===id&&i.personalization===personalization); const current=existing?.qty||0; const next=Math.min(current+qty,p.available_stock);
    if(existing) existing.qty=next; else state.cart.push({id,qty:Math.min(qty,p.available_stock),personalization});
    saveCart(); toast(`${p.nombre} agregado al carrito`);
  }
  function changeCart(index,delta){ const item=state.cart[index], p=state.data.products.find(x=>x.id===item.id); if(!item||!p)return; item.qty=Math.max(0,Math.min(item.qty+delta,p.available_stock)); if(item.qty===0)state.cart.splice(index,1); saveCart(); }
  function renderCart(){
    ...
    else body.innerHTML=state.cart.map((item,i)=>{ const p=state.data.products.find(x=>x.id===item.id); if(!p)return''; subtotal+=p.precio*item.qty; count+=item.qty; const img=imagesFor(p)[0]; return `<div class="cart-line"><div class="cart-thumb">${img?`<img src="${escapeAttr(img)}" alt="">`:productEmoji(p)}</div><div><h4>${escapeHtml(p.nombre)}</h4>${item.personalization?`<small>${escapeHtml(item.personalization)}</small>`:''}<div class="cart-actions"><button data-cart-minus="${i}">−</button><strong>${item.qty}</strong><button data-cart-plus="${i}">+</button><button class="remove-link" data-cart-remove="${i}">Quitar</button></div></div><div class="line-price">${money(p.precio*item.qty)}</div></div>`; }).join('');
    $('#cartCount').textContent=count; $('#cartSubtotal').textContent=money(subtotal); $('#checkoutSubtotal').textContent=money(subtotal); updateCheckoutTotals();
    $$('[data-cart-minus]').forEach(b=>b.onclick=()=>changeCart(+b.dataset.cartMinus,-1)); $$('[data-cart-plus]').forEach(b=>b.onclick=()=>changeCart(+b.dataset.cartPlus,1)); $$('[data-cart-remove]').forEach(b=>b.onclick=()=>{state.cart.splice(+b.dataset.cartRemove,1);saveCart();});
  }
  ...
  function openProduct(id){
    const p=state.data.products.find(x=>x.id===id); if(!p)return; state.modalProduct=p;state.modalQty=1;
    $('#modalName').textContent=p.nombre; $('#modalPrice').textContent=money(p.precio); $('#modalDescription').textContent=p.descripcion||''; $('#modalCategory').textContent=categoryById(p.categoria_id)?.nombre||'Detalle'; $('#modalStock').textContent=p.available_stock>0?`${p.available_stock} disponibles ahora`:'Agotado'; $('#modalQty').textContent='1'; $('#modalPersonalization').value=''; $('#modalAdd').disabled=p.available_stock<=0;
    const imgs=imagesFor(p), main=$('#modalMainImage'); main.innerHTML=imgs.length?`<img src="${escapeAttr(imgs[0])}" alt="${escapeAttr(p.nombre)}">`:`<div class="product-fallback"><span class="emoji">${productEmoji(p)}</span><small>Agrega las fotos reales desde administración</small></div>`;
  ...
  function renderZones(){ const sel=$('#zoneSelect'); if(!sel)return; sel.innerHTML='<option value="">Selecciona</option>'+state.data.delivery.filter(d=>d.activo).sort((a,b)=>toNum(a.orden)-toNum(b.orden)).map(d=>`<option value="${escapeAttr(d.zona_id)}">${escapeHtml(d.nombre)}</option>`).join(''); }
  function selectedZone(){ return state.data.delivery.find(d=>d.zona_id===$('#zoneSelect').value); }
  function updateCheckoutTotals(){ const s=subtotal(), z=selectedZone(); let delivery=null; if(z&&!z.requiere_cotizacion)delivery=z.costo; $('#checkoutSubtotal').textContent=money(s); $('#checkoutDelivery').textContent=delivery===null?'Por confirmar':money(delivery); $('#checkoutTotal').textContent=delivery===null?`${money(s)} + delivery`:money(s+delivery); $('#deliveryNote').textContent=z?.nota||'El costo final de delivery puede requerir confirmación según la dirección.'; }
  function setMinDate(){ const d=new Date(); d.setHours(d.getHours()+Number(state.data.config.MIN_NOTICE_HOURS||6)); const input=$('[name="deliveryDate"]'); if(input)input.min=d.toISOString().slice(0,10); }
  function openCheckout(){ if(!state.cart.length)return toast('Agrega al menos un producto.'); closeLayer($('#cartDrawer')); setTimeout(()=>openLayer($('#checkoutModal')),180); updateCheckoutTotals(); }
  function makeMapsUrl(){ const form=new FormData($('#checkoutForm')); const q=[form.get('address'),selectedZone()?.distrito||'',cfg.DEFAULT_CITY||'Piura',cfg.COUNTRY||'Perú'].filter(Boolean).join(', '); return `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(q)}`; }
  function makeWhatsApp(number,reservation,payload){
    if(!number) return '';
    const lines=[`Hola, quiero confirmar mi pedido ${reservation.reserva_id||reservation.id||''}.`,'',...payload.items.map(i=>`• ${i.name} x${i.qty} — ${money(i.unitPrice*i.qty)}`),'',`Productos: ${money(reservation.subtotal??payload.subtotal)}`,`Delivery: ${reservation.delivery===null||reservation.delivery===undefined?'por confirmar':money(reservation.delivery)}`,`Total estimado: ${reservation.total?money(reservation.total):money(payload.subtotal)+' + delivery'}`,'',`Entrega: ${payload.deliveryDate} · ${payload.deliveryWindow}`,`Dirección: ${payload.address}`,`Zona: ${payload.zoneName}`,payload.mapsUrl?`Ubicación: ${payload.mapsUrl}`:'',payload.dedication?`Dedicatoria: ${payload.dedication}`:'',payload.notes?`Observaciones: ${payload.notes}`:''].filter(v=>v!==''||true);
    return `https://wa.me/${String(number).replace(/\D/g,'')}?text=${encodeURIComponent(lines.join('\n'))}`;
  }

  async function reserve(e){
    e.preventDefault(); const btn=$('#reserveButton'),msg=$('#checkoutMessage'); msg.hidden=true;
    if(!state.cart.length)return toast('Tu carrito está vacío.');
    const fd=new FormData(e.currentTarget), z=selectedZone(); if(!z)return;
    ...
    const payload={name:String(fd.get('name')||'').trim(),phone:String(fd.get('phone')||'').trim(),email:String(fd.get('email')||'').trim(),deliveryDate:fd.get('deliveryDate'),deliveryWindow:fd.get('deliveryWindow'),zoneId:z.zona_id,zoneName:z.nombre,address:String(fd.get('address')||'').trim(),reference:String(fd.get('reference')||'').trim(),mapsUrl:String(fd.get('mapsUrl')||'').trim()||makeMapsUrl(),dedication:String(fd.get('dedication')||'').trim(),notes:String(fd.get('notes')||'').trim(),items,subtotal:subtotal(),origin:location.href};
    btn.disabled=true;btn.textContent='Verificando stock…';
    try{
      let result;
      if(state.apiReady){ result=await apiPost('reserve',payload); if(result.ok===false) throw new Error(result.error||'No se pudo separar el pedido.'); result=result.data||result; }
      else { result={reserva_id:`DEMO-${Date.now().toString().slice(-6)}`,subtotal:payload.subtotal,delivery:z.requiere_cotizacion?null:z.costo,total:z.requiere_cotizacion?null:payload.subtotal+z.costo,demo:true}; }
      const number=result.whatsapp_number||state.data.config.WHATSAPP_NUMBER||cfg.PUBLIC_WHATSAPP; const wa=makeWhatsApp(number,result,payload);
      if(!state.apiReady){ msg.hidden=false; msg.textContent='La tienda está lista visualmente, pero la reserva real de stock se activará al conectar el Web App de Google Apps Script.'; if(!number) return; }
      else { state.cart=[];saveCart(); }
      toast(`Reserva ${result.reserva_id} creada`);
      if(wa) setTimeout(()=>window.open(wa,'_blank','noopener'),350); else { msg.hidden=false;msg.textContent=`Reserva ${result.reserva_id} registrada. Falta configurar el número de WhatsApp de la tienda.`; }
    }catch(err){ msg.hidden=false;msg.textContent=err.message||'Ocurrió un problema al separar el pedido.'; }
    finally{btn.disabled=false;btn.textContent='Separar productos y continuar por WhatsApp →';}
  }
  ...
  function bind(){
    $('#cartOpen').onclick=openCart; $('#checkoutStart').onclick=openCheckout; $('#overlay').onclick=()=>{$$('.open.drawer,.open.modal').forEach(closeLayer)};
    ...
    $('#searchInput').oninput=e=>{state.search=e.target.value.trim();renderProducts()}; $('#sortSelect').onchange=e=>{state.sort=e.target.value;renderProducts()}; $('#showAllCategories').onclick=()=>{state.category='';renderCategories();renderProducts();$('#productos').scrollIntoView({behavior:'smooth'})};
    ...
    $('#modalAdd').onclick=()=>{if(!state.modalProduct)return;addToCart(state.modalProduct.id,state.modalQty,$('#modalPersonalization').value.trim());closeLayer($('#productModal'));openCart();};
    ...
    $('#useLocation').onclick=()=>{ const status=$('#locationStatus'); if(!navigator.geolocation){status.textContent='Tu navegador no permite compartir ubicación.';return;} status.textContent='Obteniendo ubicación…'; navigator.geolocation.getCurrentPosition(pos=>{ const u=`https://www.google.com/maps?q=${pos.coords.latitude},${pos.coords.longitude}`;$('#mapsUrl').value=u;status.textContent='Ubicación agregada correctamente.'; },()=>status.textContent='No se pudo obtener la ubicación. Puedes pegar un enlace de Google Maps.'); };
    $('#customWhatsApp').onclick=()=>{ const n=state.data.config.WHATSAPP_NUMBER||cfg.PUBLIC_WHATSAPP; if(!n)return toast('Falta configurar el WhatsApp de la tienda.');window.open(`https://wa.me/${String(n).replace(/\D/g,'')}?text=${encodeURIComponent('Hola, quisiera cotizar un detalle personalizado.')}`,'_blank','noopener'); };
    document.addEventListener('keydown',e=>{if(e.key==='Escape')$$('.open.drawer,.open.modal').forEach(closeLayer)}); $('#year').textContent=new Date().getFullYear();
  }
  bind(); bootstrap();
})();
